...
def process_image4(original_image, index, row, column, i, j, row_step, column_step):  # Douglas-peucker approximation
    ...
    if cnts != ():
        ...
        for object in cnts:
            ...
            for idx in range(len(approx)):
                #print(i)
                #print(approx[idx])
                x_offset = (approx[idx][0][0] + i*row_step)* pixel_length_ratio
                y_offset = (approx[idx][0][1] + j*column_step)* pixel_length_ratio
    ...
    if cnts != ():
        for i in cnts:
            ...
            for idx in range(len(approx)):
                #print(i)
                #print(approx[idx])
                x_offset = (approx[idx][0][0])* pixel_length_ratio
                y_offset = (approx[idx][0][1])* pixel_length_ratio
                arm_x_offset = -x_offset
                arm_y_offset = y_offset
                #arm_x_offset = x_offset*math.cos(45/180*np.pi)-y_offset*math.cos(45/180*np.pi)
                #arm_y_offset = x_offset*math.cos(45/180*np.pi)+y_offset*math.cos(45/180*np.pi)
...
def screen_record(img, all_img, row, column, args=None):
    ...
    for i in range(row):
        for j in range(column):
            image = process_image4(all_img[count], count, row, column, i, j, row_step, column_step)
